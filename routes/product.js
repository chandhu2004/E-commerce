...
router.post('/upload', verifyToken, upload.single('image'), async (req, res) => {
    try {
        ...
        if (!name || !price || !stock || !image) {
            return res.status(400).json({ message: 'All fields (name, price, stock, image) are required' });
        }
        ...
        const product = {
            ...
            stock: parseInt(stock),
            description: description || '',
            imagePath: image.path, // Cloudinary URL
            user: req.user.email,
            uploadedAt: new Date()
        };
...
router.put('/update/:id', verifyToken, upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        // If new image is uploaded
        if (image) {
            updateData.imagePath = image.path; // Cloudinary URL
        }
...
/**
 * @route   GET /products
 * @desc    Render all products (public)
 */
